...
static void make_segment(uint8_t segment[20], uint32_t source,
    ...
    write16(segment + 16u, 0u);
    write16(segment + 16u, (uint16_t)sb_tcp_checksum_ipv4(source, destination, segment, 20u));
}
...
int main(void) {
    ...
    uint8_t bad_checksum[20];
    make_segment(bad_checksum, remote, local, 1234u, 4321u, 1u, 0u, SB_TCP_FLAG_SYN);
    bad_checksum[19] ^= 1u;
