...
int process_activate(sb_process_t *process) {
    return process != 0 ? (process_activate_calls++, 0) : -1;
}
...
int gdt_try_set_kernel_stack(uint64_t stack_pointer) {
    return (stack_pointer != 0u && (stack_pointer & 0xFu) == 0u)
        ? (gdt_calls++, 0) : -1;
}
...
static void setup_thread(sb_process_t *process,
                         sb_thread_t *thread,
    ...
    *thread = (sb_thread_t){
        ...
        .kernel_stack_top = base + 0x1000u,
        .kernel_resume_stack_pointer = base + 0xF00u,
    };
...
int main(void) {
    ...
    sb_x86_64_user_iret_frame_t *iret =
        (sb_x86_64_user_iret_frame_t *)(void *)(frame_storage + sizeof(*gpr));

    *iret = (sb_x86_64_user_iret_frame_t){
        .rip = 0x400000u, .cs = 0x23u, .rflags = 0x202u,
        .rsp = 0x7FFFF000u, .ss = 0x2Bu
    };
    *gpr = (sb_timer_saved_gpr_t){ .rax = 0x1234u };
    ...
    uintptr_t rsp = user_scheduler_timer_dispatch(gpr);
    assert(rsp == t2.kernel_resume_stack_pointer);
    assert(user_scheduler_current_thread() == &t2);
    assert(user_scheduler_current_process() == &p2);
    assert(c1.rip == 0x400000u);
    assert(c1.cs == 0x23u);
    assert(process_activate_calls == 1);
    assert(gdt_calls == 1);

    iret->cs = 0x18u;
    assert(user_scheduler_timer_dispatch(gpr) == 0u);
    return 0;
}
