...
sb_process_t *process_create(uint64_t pid) {
    if (process_count_value >= 16u || pid == 0u) return 0;
    for (uint32_t i = 0u; i < 16u; ++i) {
        if (processes[i].state != SB_PROCESS_UNUSED) continue;
        processes[i] = (sb_process_t){.pid = pid, .state = SB_PROCESS_CREATED};
        ++process_count_value;
        return &processes[i];
    }
    return 0;
...
sb_process_t *process_create_child(sb_process_t *parent, uint64_t pid) {
    ...
    sb_process_t *child = process_create(pid);
    if (child != 0) child->parent_pid = parent->pid;
    return child;
}
...
int process_prepare_boot_module(sb_process_t *process, uint64_t multiboot_info,
    ...
    *image = (sb_process_image_t){.entry_point = 0x400000u, .user_stack_top = 0x800000u,
                                  .user_stack_bottom = 0x7FC000u};
    return 0;
}
...
int process_prepare_elf_thread(sb_process_t *process, uint64_t tid, uint32_t priority,
    ...
    (void)priority;
    if (process == 0 || context == 0 || image == 0 || thread_out == 0 || tid == 0u) return -1;
    uint32_t index = 0u;
    while (index < 16u && &processes[index] != process) ++index;
    if (index >= 16u) return -1;
    sb_thread_t *thread = &threads[index];
    *thread = (sb_thread_t){.tid = tid, .state = SB_PROCESS_CREATED, .user_context = context,
...
int main(void) {
    ...
    assert(sb_app_is_running(SB_APP_SETTINGS) == 1);
    assert(sb_app_launch(SB_APP_SETTINGS) != 0);
    ...
    assert(sb_app_terminate(SB_APP_FILES, 17u) == 0);
    assert(sb_app_is_running(SB_APP_FILES) == 0);
    assert(sb_app_count() == 3u);
    assert(sb_app_terminate(SB_APP_FILES, 18u) != 0);
    assert(sb_app_count() == 2u);

    fail_terminate = 1;
    assert(sb_app_terminate(SB_APP_TERMINAL, 19u) != 0);
    assert(sb_app_is_running(SB_APP_TERMINAL) == 1);
    assert(terminate_calls == 2u);
    fail_terminate = 0;
    assert(sb_app_terminate(SB_APP_TERMINAL, 20u) == 0);
    assert(sb_app_is_running(SB_APP_TERMINAL) == 0);
    assert(sb_app_count() == 2u);
    ...
    assert(sb_app_launch(SB_APP_FILES) == 0);
    assert(sb_app_count() == 2u);
    assert(sb_app_terminate(SB_APP_SETTINGS, 21u) == 0);
    assert(sb_app_reap_exited() == 1u);
    assert(sb_app_count() == 1u);
    assert(sb_app_launch(SB_APP_SETTINGS) == 0);
    assert(sb_app_count() == 2u);
    ...
    sb_process_t parent = {.pid = 0xABCDEF1234567890ull, .state = SB_PROCESS_RUNNING};
    processes[0] = parent;
    ...
    sb_process_t outsider = {.pid = 0x1234567890ABCDEFuLL, .state = SB_PROCESS_RUNNING};
