...
typedef int (*sb_resource_cache_begin_fn)(void *user,
                                          ...
                                          void **transaction);
typedef int (*sb_resource_cache_write_fn)(void *transaction,
                                          const uint8_t *data,
                                          uint32_t size);
typedef int (*sb_resource_cache_commit_fn)(void *transaction,
                                           const sb_resource_ref_t *ref,
                                           const char *sha256);
typedef void (*sb_resource_cache_abort_fn)(void *transaction);
typedef int (*sb_resource_activate_fn)(void *user,
                                       const sb_resource_ref_t *ref,
                                       const char *sha256);
typedef int (*sb_resource_fetch_fn)(void *user,
                                    const char *path,
                                    int (*emit)(void *emit_user,
                                                const uint8_t *data,
                                                uint32_t size),
                                    void *emit_user);
