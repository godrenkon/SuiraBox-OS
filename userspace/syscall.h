...
uint64_t sb_fs_mkdir(const char *path, uint32_t path_length);
...
static inline uint64_t sb_fs_mkdir(const char *path,uint32_t path_length){return sb_syscall2(SB_SYS_FS_MKDIR,(uint64_t)(uintptr_t)path,path_length);}
