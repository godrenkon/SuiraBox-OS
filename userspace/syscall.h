...
static inline uint64_t sb_display_rect(uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t rgb) { uint64_t result; register uint64_t rdi __asm__("rdi") = x; register uint64_t rsi __asm__("rsi") = y; register uint64_t rdx __asm__("rdx") = width; register uint64_t r10 __asm__("r10") = height; register uint64_t r8 __asm__("r8") = rgb; __asm__ volatile ("int $0x80" : "=a"(result) : "a"(SB_SYS_DISPLAY_RECT), "D"(rdi), "S"(rsi), "d"(rdx), "r"(r10), "r"(r8) : "memory"); return result; }
...
static inline uint64_t sb_display_glyph(uint32_t x, uint32_t y, uint64_t bitmap, uint32_t rgb) { uint64_t result; register uint64_t rdi __asm__("rdi") = x; register uint64_t rsi __asm__("rsi") = y; register uint64_t rdx __asm__("rdx") = bitmap; register uint64_t r10 __asm__("r10") = rgb; __asm__ volatile ("int $0x80" : "=a"(result) : "a"(SB_SYS_DISPLAY_GLYPH), "D"(rdi), "S"(rsi), "d"(rdx), "r"(r10) : "memory"); return result; }
static inline uint64_t sb_display_glyph_pair(uint32_t x, uint32_t y, uint64_t bitmap_a, uint64_t bitmap_b, uint32_t rgb) { uint64_t result; register uint64_t rdi __asm__("rdi") = x; register uint64_t rsi __asm__("rsi") = y; register uint64_t rdx __asm__("rdx") = bitmap_a; register uint64_t r10 __asm__("r10") = bitmap_b; register uint64_t r8 __asm__("r8") = rgb; __asm__ volatile ("int $0x80" : "=a"(result) : "a"(SB_SYS_DISPLAY_GLYPH_PAIR), "D"(rdi), "S"(rsi), "d"(rdx), "r"(r10), "r"(r8) : "memory"); return result; }
