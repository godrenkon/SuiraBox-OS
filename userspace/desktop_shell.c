...
static uint32_t launcher_menu_height(const sb_desktop_shell_t *shell) {
    if (shell == 0 || shell->launcher.count == 0u || shell->launcher.count > UINT32_MAX / SB_SHELL_MENU_ROW_H) return 0u;
...
static int launcher_menu_top(const sb_desktop_shell_t *shell, int32_t *top) {
    ...
    if (shell == 0 || top == 0 || menu_height == 0u || shell->screen_height < SB_GUI_TASKBAR_HEIGHT || shell->screen_height > (uint32_t)INT32_MAX) return -1;
...
void sb_desktop_shell_init(sb_desktop_shell_t *shell, uint32_t screen_width, uint32_t screen_height) {
    if (shell == 0) return;
...
int sb_desktop_shell_register_default_apps(sb_desktop_shell_t *shell) {
    if (shell == 0 || shell->initialized == 0u || shell->launcher.count != 0u) return -1;
...
#if __STDC_HOSTED__ == 0
    if (request_launcher_app(id) != 0) {
        shell->launcher.open = 1u;
        return -1;
    }
#endif
    *activated_id = id;
...
int sb_desktop_shell_key(sb_desktop_shell_t *shell, uint8_t key) {
    ...
    if (key == 0x1Cu) return sb_desktop_shell_activate_selected(shell, &activated_id);
    return -1;
}

int sb_desktop_shell_click(sb_desktop_shell_t *shell, int32_t x, int32_t y, const char **activated_id) {
    uint32_t index;
    int32_t menu_top;
    if (activated_id != 0) *activated_id = 0;
    if (shell == 0 || shell->initialized == 0u || activated_id == 0 || shell->screen_height < SB_GUI_TASKBAR_HEIGHT || shell->screen_height > INT32_MAX) return -1;
    const int32_t taskbar_top = (int32_t)shell->screen_height - (int32_t)SB_GUI_TASKBAR_HEIGHT;
    if (x >= (int32_t)SB_SHELL_LAUNCHER_X && x < (int32_t)(SB_SHELL_LAUNCHER_X + SB_SHELL_LAUNCHER_W) && y >= taskbar_top && y < (int32_t)shell->screen_height)
        return sb_desktop_shell_toggle_launcher(shell);
    if (shell->launcher.open == 0u || launcher_menu_top(shell, &menu_top) != 0) return -1;
    if (sb_launcher_hit_test(&shell->launcher, x, y, SB_SHELL_LAUNCHER_X, (uint32_t)menu_top, SB_SHELL_MENU_W, SB_SHELL_MENU_ROW_H, &index) != 0) return -1;
...
void sb_desktop_shell_present_launcher(void) {
    ...
    int32_t top;
    const uint32_t menu_height = launcher_menu_height(shell);
    if (shell == 0 || shell->initialized == 0u || shell->launcher.open == 0u || menu_height == 0u || launcher_menu_top(shell, &top) != 0) return;
    ...
    for (uint32_t i = 0u; i < shell->launcher.count; ++i) {
        ...
        draw_rect(SB_SHELL_LAUNCHER_X + 2u, row_y + 2u, SB_SHELL_MENU_W - 4u, SB_SHELL_MENU_ROW_H - 4u, i == shell->launcher.selected ? 0x536F8Au : 0x27313Eu);
