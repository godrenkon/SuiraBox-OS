...
int sb_resource_manager_init(sb_resource_manager_t *manager,
    ...
    for (uint32_t i = 0u; i < manifest_count; ++i) {
        if (manifest[i].tier == SB_RESOURCE_TIER_REMOTE) {
            has_remote = 1u;
            break;
        }
    }
    if (has_remote != 0u && io->verify_manifest == 0) return -3;
    if (has_remote != 0u && io->verify_manifest(io->user, manifest, manifest_count) != 0) return -4;
