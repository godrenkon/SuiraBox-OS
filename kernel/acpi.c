...
static uint8_t checksum8(const uint8_t *data, uint32_t length) { uint8_t sum = 0u; for (uint32_t i = 0u; i < length; ++i) sum = (uint8_t)(sum + data[i]); return sum; }
static uint32_t rd32(const void *ptr) { const uint8_t *p = (const uint8_t *)ptr; return (uint32_t)p[0] | ((uint32_t)p[1] << 8) | ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24); }
static uint64_t rd64(const void *ptr) { const uint8_t *p = (const uint8_t *)ptr; uint64_t v = 0u; for (uint32_t i = 0u; i < 8u; ++i) v |= (uint64_t)p[i] << (8u * i); return v; }
static int signature_is(const void *ptr, const char *signature, uint32_t length) { const uint8_t *p = (const uint8_t *)ptr; for (uint32_t i = 0u; i < length; ++i) if (p[i] != (uint8_t)signature[i]) return 0; return 1; }
...
static const uint8_t *valid_table(uint64_t address, const char *signature, uint32_t *length_out) {
    ...
    if (length < ACPI_TABLE_HEADER_SIZE || length > ACPI_MAX_TABLE_SIZE || checksum8(table, length) != 0u) return 0;
    *length_out = length;
    return table;
}
static uint64_t find_table_in_root(uint64_t root_address, const char *root_signature, const char *wanted_signature) {
    ...
    if (((root_length - ACPI_TABLE_HEADER_SIZE) % entry_size) != 0u) return 0u;
            ...
            if (rsdp_validate((const uint8_t *)tag + sizeof(struct mb_tag), tag->size - sizeof(struct mb_tag), &info) == 0) {
                ...
                if (fadt != 0u) {
                    ...
                    if (table != 0) {
                        ...
                        info.dsdt_address = x_dsdt != 0u ? x_dsdt : (flen >= 44u ? (uint64_t)rd32(table + 40u) : 0u);
                        info.pm1a_control_block = fadt_gas_io(table, flen, 172u, 64u);
                        info.pm1b_control_block = fadt_gas_io(table, flen, 184u, 68u);
                        info.pm1_control_length = flen >= 90u ? table[89] : 0u;
                        if (info.dsdt_address != 0u && extract_s5_types(info.dsdt_address, &info.sleep_type_a, &info.sleep_type_b) == 0 && info.pm1a_control_block != 0u && info.pm1_control_length >= 2u)
