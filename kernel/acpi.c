...
static int rsdp_validate(const void *ptr, uint32_t available, sb_acpi_info_t *out) {
    ...
    for (uint32_t i = 0u; i < sizeof(signature); ++i)
    ...
    if (revision >= 2u) {
        ...
        if (length < ACPI_RSDP_V2_LENGTH || length > available) return -1;
        if (checksum8(bytes, length) != 0u) return -1;
    }
...
int sb_acpi_init_from_multiboot(uint64_t multiboot_info) {
    ...
    while (offset <= total_size - sizeof(struct mb_tag)) {
        ...
        if (tag->type == MB_TAG_END) return -1;
        if (tag->type == MB_TAG_ACPI_OLD || tag->type == MB_TAG_ACPI_NEW) {
            if (tag->size <= sizeof(struct mb_tag)) return -1;
            if (rsdp_validate((const uint8_t *)tag + sizeof(struct mb_tag),
                              tag->size - sizeof(struct mb_tag), &info) == 0)
                return 0;
        }
