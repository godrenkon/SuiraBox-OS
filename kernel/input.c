...
static void enqueue_event(sb_input_event_type_t type, uint8_t a, uint8_t b, uint8_t c, uint8_t d) {
    ...
    event_queue[event_head] = (sb_input_event_t){
        .sequence = ++event_sequence, .type = type,
...
uint64_t sb_input_read_key(void) {
    ...
    const uint8_t scancode = io_in8(0x60u);
    enqueue_event(SB_INPUT_EVENT_KEY, scancode, 0u, 0u, 0u);
    return scancode;
}
...
uint64_t sb_input_read_mouse(void) {
    ...
    return 1u | ((uint64_t)(mouse_packet[0] & 0x07u) << 8) |
           ((uint64_t)mouse_packet[1] << 16) | ((uint64_t)mouse_packet[2] << 24);
}

int sb_input_poll_event(sb_input_event_t *event) {
    if (event == 0 || event_count == 0u) return 0;
