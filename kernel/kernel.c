...
static int storage_selftest(void) {
    ...
    if (sb_vfs_write_sectors(&mount, 3, 1, write_buffer) != SB_VFS_OK ||
        sb_vfs_read_sectors(&mount, 3, 1, read_buffer) != SB_VFS_OK) return 0;
    for (uint32_t i = 0; i < SB_BLOCK_SECTOR_SIZE; ++i)
...
static int real_disk_selftest(void) {
    ...
    for (uint32_t i = 0; marker[i] != '\0'; ++i)
        if (buffer[i] != (uint8_t)marker[i]) return 0;
    return 1;
}
...
static int vmm_selftest(void) {
    ...
    if (translated != (uint64_t)(uintptr_t)page) {
        uint64_t discarded; (void)vmm_unmap_page(test_virtual, &discarded); pmm_free_page(page); return 0;
    }
...
static int heap_selftest(void) {
    ...
    for (uint32_t i = 0; i < 128u; ++i) {
        if (memory[i] != (uint8_t)(i ^ 0xA5u)) { kheap_free(memory); return 0; }
    }
    kheap_free(memory);
...
void kmain(uint64_t multiboot_magic, uint64_t multiboot_info) {
    ...
    if ((uint32_t)multiboot_magic == 0x36D76289u) serial_write("Boot protocol: Multiboot2 OK\r\n");
    else serial_write("Boot protocol: unexpected magic\r\n");
    ...
    serial_write("Memory: PMM total pages = "); serial_write_u64(pmm_total_pages()); serial_write("\r\n");
    serial_write("Memory: PMM free pages = "); serial_write_u64(pmm_free_pages()); serial_write("\r\n");
    void *page = pmm_alloc_page();
    if (page != 0) { serial_write("Memory: page allocation OK\r\n"); pmm_free_page(page); serial_write("Memory: page free OK\r\n"); }
    else serial_write("Memory: page allocation FAILED\r\n");
    ...
    serial_write("Interrupts: initializing IDT...\r\n");
    interrupts_init();
    serial_write("Interrupts: IDT loaded\r\n");
    serial_write("Timer: initializing PIT at 100 Hz...\r\n");
    timer_init(100u);
    serial_write("Timer: interrupts enabled\r\n");
    uint64_t start_ticks = timer_ticks();
    while (timer_ticks() < start_ticks + 3u) __asm__ volatile ("hlt");
    serial_write("Timer: tick test OK, ticks = "); serial_write_u64(timer_ticks()); serial_write("\r\n");
