...
static int heap_selftest(void) {
    ...
    memory[2] = 0x3Cu;
    memory[3] = 0xC3u;
    if (memory[0] != 0xA5u || memory[1] != 0x5Au ||
        memory[2] != 0x3Cu || memory[3] != 0xC3u) {
        kheap_free(memory);
        return 0;
    }
    kheap_free(memory);
...
static int scheduler_selftest(void) {
    ...
    if (scheduler_add_kernel_task(2u, 128u) != 0 ||
        ...
        scheduler_task_count() != 3u) return 0;
...
void kmain(uint64_t multiboot_magic, uint64_t multiboot_info) {
    ...
    serial_write((uint32_t)multiboot_magic == 0x36D76289u
    ...
    if (sb_framebuffer_init(multiboot_info)) {
        ...
        serial_write(" ");
        serial_write_u64(fb->bits_per_pixel);
        serial_write("bpp\r\n");
    } else {
    ...
    serial_write(sb_storage_selftest()
    ...
    serial_write("Memory: Multiboot info = ");
    serial_write_u64(multiboot_info);
    serial_write("\r\n");
    report_multiboot_modules(multiboot_info);
    serial_write("Memory: PMM bootstrap free pages = ");
