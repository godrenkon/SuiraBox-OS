...
static void dump_interrupt_state(void) {
    struct debug_desc_ptr gdtr,idtr; struct debug_idt_entry entry; uint16_t cs;
    __asm__ volatile ("sgdt %0" : "=m"(gdtr)); __asm__ volatile ("sidt %0" : "=m"(idtr)); __asm__ volatile ("mov %%cs, %0" : "=r"(cs));
    const struct debug_idt_entry *idt=(const struct debug_idt_entry *)(uintptr_t)idtr.base; entry=idt[32];
...
void timer_init_frequency(uint32_t frequency_hz) {
    timer_debug("[TIMER] init begin\r\n"); interrupts_disable(); if(frequency_hz==0u)frequency_hz=SB_TIMER_DEFAULT_HZ; uint32_t divisor=PIT_BASE_HZ/frequency_hz; if(divisor==0u)divisor=1u;if(divisor>0xFFFFu)divisor=0xFFFFu; ticks=0;
    timer_debug("[TIMER] PIC remap begin\r\n");pic_remap();timer_debug("[TIMER] PIC remap complete\r\n");timer_debug("[TIMER] PIT program begin\r\n");outb(PIT_COMMAND,0x36u);outb(PIT_CHANNEL0,(uint8_t)(divisor&0xFFu));outb(PIT_CHANNEL0,(uint8_t)((divisor>>8)&0xFFu));timer_debug("[TIMER] PIT program complete\r\n");timer_debug("[TIMER] IRQ0 handler install begin\r\n");interrupts_set_handler(32u,(uintptr_t)sb_timer_irq_stub);timer_debug("[TIMER] IRQ0 handler install complete\r\n");dump_interrupt_state();timer_debug("[TIMER] software int 0x20 begin\r\n");__asm__ volatile("int $0x20":::"memory");timer_debug("[TIMER] software int 0x20 returned\r\n");timer_debug("[TIMER] STI begin\r\n");interrupts_enable();timer_debug("[TIMER] STI returned\r\n");
}
void timer_init(void){timer_init_frequency(SB_TIMER_DEFAULT_HZ);} uint64_t timer_ticks(void){return ticks;}
...
uintptr_t sb_timer_irq_dispatch(sb_timer_saved_gpr_t *gpr) {
    ...
    if((interrupted_cs&3u)==3u){
        ...
        if(current_thread != 0 && current_process != 0 &&
           (current_thread->state == SB_PROCESS_CREATED || current_thread->state == SB_PROCESS_RUNNING)) {
            if(current_thread->runtime_ticks != UINT64_MAX) ++current_thread->runtime_ticks;
            if(current_process->runtime_ticks != UINT64_MAX) ++current_process->runtime_ticks;
        }
