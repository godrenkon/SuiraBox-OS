...
static int map_user_stack(sb_address_space_t *space,
    ...
    for (uint64_t va = bottom; va < top; va += SB_PAGE_SIZE) {
        ...
        if (page == 0) return -1;
        for (uint32_t i = 0u; i < SB_PAGE_SIZE; ++i) ((uint8_t *)page)[i] = 0u;
        if (address_space_map_user(space, va, (uint64_t)(uintptr_t)page,
...
static void process_prepare_elf_rollback(sb_process_t *process) {
    if (process == 0) return;
    if (process->address_space.pml4_physical != 0u) address_space_destroy(&process->address_space);
...
int process_prepare_elf(sb_process_t *process,
    ...
    *image_info = (sb_process_image_t){0};
    if (process->address_space.pml4_physical == 0u && address_space_create(&process->address_space) != 0) return -1;
    uint64_t entry = 0u;
...
int process_prepare_elf_thread(sb_process_t *process,
    ...
    if (process == 0 || context == 0 || image_info == 0 || thread_out == 0 ||
        ...
        process->state == SB_PROCESS_UNUSED || process->state == SB_PROCESS_EXITED) return -1;
    *thread_out = 0;
    if (sb_user_context_init(context, image_info->entry_point, image_info->user_stack_top) != 0) return -1;
