...
static uint64_t pending_sleep_wake_tick;
...
static void prune_dead_slots(void) {
    uint32_t i = 0u;
    while (i < slot_count) {
        const sb_thread_t *thread = slots[i].thread;
        const sb_process_t *process = slots[i].process;
        const int pending_exit = pending_exit_process == process && pending_exit_thread == thread;
        const int dead = thread == 0 || process == 0 ||
                         thread->state == SB_PROCESS_UNUSED ||
                         thread->state == SB_PROCESS_EXITED ||
                         process->state == SB_PROCESS_UNUSED ||
                         process->state == SB_PROCESS_EXITED;
        if (!pending_exit && dead) remove_slot_at(i);
        else ++i;
    }
}

static int find_current_slot(sb_process_t *process, sb_thread_t *thread, uint32_t *index) {
    if (process == 0 || thread == 0 || index == 0 ||
        slot_count == 0u || current_index >= slot_count) return -1;
    if (!slot_matches(&slots[current_index], process, thread)) return -1;
    *index = current_index;
    return 0;
}

static int find_next_runnable(uint32_t old_index, uint32_t *candidate_index) {
    if (candidate_index == 0 || slot_count < 2u || old_index >= slot_count) return -1;
    for (uint32_t step = 1u; step <= slot_count; ++step) {
        ...
        if (candidate == old_index) continue;
...
int user_scheduler_remove(sb_process_t *process, sb_thread_t *thread) {
    for (uint32_t i = 0u; i < slot_count; ++i) {
        if (!slot_matches(&slots[i], process, thread)) continue;
        remove_slot_at(i);
...
int user_scheduler_rebind_thread(sb_process_t *process,
                                 sb_thread_t *old_thread,
                                 sb_thread_t *new_thread) {
...
int user_scheduler_request_exit(sb_process_t *process, sb_thread_t *thread) {
    ...
    if (pending_exit_process != 0 || pending_exit_thread != 0 || pending_sleep_wake_tick != 0u) return -2;
...
int user_scheduler_sleep_thread(sb_process_t *process, sb_thread_t *thread, uint64_t wake_tick) {
    if (process == 0 || thread == 0 || wake_tick == 0u) return -1;
    if (!process_runnable(process)) return -2;
    if (pending_sleep_wake_tick != 0u) return -6;
    for (uint32_t i = 0u; i < slot_count; ++i) {
        if (!slot_matches(&slots[i], process, thread)) continue;
        if (i == current_index) return -3;
        if (!thread_runnable(thread)) return -4;
        thread->state = SB_PROCESS_SLEEPING;
...
int user_scheduler_request_sleep(uint64_t wake_tick) {
    sb_process_t *process;
    sb_thread_t *thread;
    uint32_t candidate_index;
    if (wake_tick == 0u || pending_sleep_wake_tick != 0u ||
        pending_exit_process != 0 || pending_exit_thread != 0) return -1;
    process = user_scheduler_current_process();
    thread = user_scheduler_current_thread();
    if (process == 0 || thread == 0 || !thread_runnable(thread) || !process_runnable(process)) return -2;
    if (find_next_runnable(current_index, &candidate_index) != 0) return -3;
    pending_sleep_wake_tick = wake_tick;
...
uint32_t user_scheduler_wake_expired(uint64_t now_tick) {
    uint32_t woke = 0u;
    for (uint32_t i = 0u; i < slot_count; ++i) {
        sb_thread_t *thread = slots[i].thread;
        sb_process_t *process = slots[i].process;
        if (thread == 0 || process == 0 || thread->state != SB_PROCESS_SLEEPING) continue;
        if (!process_runnable(process) || thread->wake_tick == 0u) continue;
        if (now_tick < thread->wake_tick) continue;
        thread->wake_tick = 0u;
...
static int frame_is_user_mode(const sb_timer_saved_gpr_t *gpr,
    ...
    const uint64_t *hardware_frame =
        (const uint64_t *)((const uint8_t *)gpr + sizeof(*gpr));
...
uintptr_t user_scheduler_timer_dispatch(sb_timer_saved_gpr_t *gpr) {
    ...
    for (uint32_t step = 1u; step <= slot_count; ++step) {
        ...
        if (!thread_runnable(next_thread) || !process_runnable(next_process) ||
            next_thread == current_thread) continue;
        ...
        current_thread->wake_tick = 0u;
        next_thread->state = SB_PROCESS_RUNNING;
        next_thread->wake_tick = 0u;
        if (current_process != next_process) current_process->state = SB_PROCESS_CREATED;
        next_process->state = SB_PROCESS_RUNNING;
