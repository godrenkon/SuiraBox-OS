...
static void set_page_range_bits(uint64_t first_page, uint64_t last_page, int make_free) {
    uint32_t first_word;
    uint32_t last_word;

    if (first_page >= PMM_MAX_PAGES || last_page <= first_page) return;
...
static uint64_t page_round_up(uint64_t address) {
    if (address > UINT64_MAX - (SB_PAGE_SIZE - 1u)) return UINT64_MAX / SB_PAGE_SIZE;
    return (address + SB_PAGE_SIZE - 1u) / SB_PAGE_SIZE;
}
...
void pmm_reset(void) {
    ...
    /* A physical address of zero is also the PMM allocation-failure sentinel. */
    reserved_bitmap[0] |= 1u;
...
void pmm_add_usable_range(uint64_t usable_start, uint64_t usable_end) {
    ...
    set_page_range_bits(page_round_up(usable_start), usable_end / SB_PAGE_SIZE, 1);
    recompute_free_count();
}

void pmm_reserve_range(uint64_t start, uint64_t end) {
    if (end <= start) return;
    set_page_range_bits(start / SB_PAGE_SIZE, page_round_up(end), 0);
    recompute_free_count();
}
