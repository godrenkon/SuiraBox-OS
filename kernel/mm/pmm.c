...
static void set_range(uint64_t start, uint64_t end, int free_pages) {
    ...
    const uint64_t first = start / SB_PAGE_SIZE;
    ...
    for (uint64_t page = first; page < last; ++page) {
        ...
        if (free_pages) bitmap[word] &= ~mask;
...
void pmm_reset(void) {
    /* 256 words for the fixed 64 MiB bootstrap arena: deliberately tiny. */
...
void pmm_add_usable_range(uint64_t usable_start, uint64_t usable_end) {
    set_range(usable_start, usable_end, 1);
}

void pmm_reserve_range(uint64_t start, uint64_t end) {
    set_range(start, end, 0);
}
